...
<center>
	...
	<%
		...
		//--------------------------------------DataBase Create and Link--------------------------------
			...
			ResultSet rs=st.executeQuery(" select * from User01 where uname='"+uname+"'    ");
			while(rs.next()!=false){
				 uuname=rs.getString("uname");
				 uupass=rs.getString("pass");
			}
			...
			if(uname.equals(uuname) && old_pass.equals(uupass)){
				...
				String s="update  User01 set pass='"+cnfm_pass+"' where uname='"+uname+"' ";
				...
				if(i>=0){
					...
					response.setHeader("refresh", "5;URL=Change_Password_Include_01.jsp");
				}
				else{
